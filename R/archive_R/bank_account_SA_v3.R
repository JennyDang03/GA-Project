...
dat <- read_dta(file.path(dta_path,"/Pix_bank_account_bf.dta"))
setDT(dat)
...
png(file.path(output_path,"/bank_accounts_pix_bf.png"), width = 640*4, height = 480*4, res = 200)
...
################################################################################
# Conditional on bf and having a bank account in October
dat <- read_dta(file.path(dta_path,"/Pix_bank_account.dta"))
setDT(dat)
dat[, treat := ifelse(is.na(date_first_pix_sent), 0, 1)]
dat[, time_to_treat := ifelse(treat==1, time_id - date_first_pix_sent, 0)]

dat <- dat[bf==1]
dat <- dat[accounts_oct_2020>0]
...
png(file.path(output_path,"/bank_accounts_pix_bf_oneaccount.png"), width = 640*4, height = 480*4, res = 200)
iplot(list(mod_twfe, mod_sa), sep = 0.5, ref.line = -1,
      xlab = 'Time to treatment',
      main = 'Event study: Pix Adoption on Bank Accounts',
      ci_level = 0.90)
...
# Use nov as the event. -> 730 is ym(2020,11)
...
dat[, time_to_treat := ifelse(treat==1, time_id - 730, 0)]
...
png(file.path(output_path,"/bank_accounts_time_id_muni.png"), width = 640*4, height = 480*4, res = 200)
iplot(mod_twfe, 
      xlab = 'Time to treatment', 
      main = 'Event study: Pix on Bank Accounts',
      ci_level = 0.90)
dev.off()
...
png(file.path(output_path,"/bank_accounts_id.png"), width = 640*4, height = 480*4, res = 200)
iplot(mod_twfe, 
      xlab = 'Time to treatment', 
      main = 'Event study: Pix on Bank Accounts',
      ci_level = 0.90)
...
################################################################################
# Eliminate data from before Nov 2020?

# Create graphs of n_accounts over time

# Condition on having at least one bank account on November 2020. 
################################################################################