...
WITH IDs AS (
  SELECT 
    ...
    tipo2
  FROM
    (SELECT TO_NUMBER(LEFT(PEF_CD_CPF, 11)) AS id, 'F' AS tipo2, 1 AS tipo 
      FROM BCBDWPRO_ACC.PESTB_PEF_PESSOA_FISICA 
      WHERE MUN_CD = @MUNI_CD_LOOP
      UNION ALL 
      SELECT TO_NUMBER(LEFT(PEJ_CD_CNPJ14, 14)) AS id, 'J' AS tipo2, 2 AS tipo 
      FROM BCBDWPRO_ACC.PESTB_PEJ_PESSOA_JURIDICA
      WHERE MUN_CD = @MUNI_CD_LOOP) AS Combined
),
MAIN AS (
  SELECT
