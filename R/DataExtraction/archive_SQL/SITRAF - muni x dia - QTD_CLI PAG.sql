-- TED por municipio RECEBEDOR = PAGADOR, dia e tipo de pessoa, Pagador
		SELECT 
				PAL_DT_MOVTO as dia,
				cadastroReceitaPag.CODIGO_DO_MUNICIPIO_NO_BCBASE AS muni_cd,
				COALESCE(tableCIPDetails.[PAT_CD_TP_PES_REM], tableCIPDetails.[PAT_CD_TP_PES_DEB]) as tipo_pessoa_PAG,
				COUNT(DISTINCT cadastroReceitaPag.CODIGO_CNPJ_14) AS Qtd_Cli_Pag
		...
		GROUP BY
		PAL_DT_MOVTO,
		cadastroReceitaPag.CODIGO_DO_MUNICIPIO_NO_BCBASE,
