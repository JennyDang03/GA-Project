...
# Pix_flow_new --------------------------------------------------------------- 
# Variables: flow_code, week, muni_cd, sender_type, receiver_type, senders, receivers, users, valor, trans, valor_w
...
# Variables: date_flood, flow_code, week, muni_cd, sender_type, receiver_type, senders, receivers, users, valor, trans, valor_w,
...
tipo <- c("bank_flow_new", "bank_flow_self_new")
...
for(i in 1:length(tipo)) {
  ...
  key_columns <- c("week", "muni_cd", "tipo", "bank")
  duplicates <- data[duplicated(data[key_columns]), ]
  data <- data %>% 
    distinct(week, muni_cd, tipo, bank, .keep_all = TRUE)
  ...
  # Download data
  write_dta(data, paste0(dta,tipo[i],".dta"))
