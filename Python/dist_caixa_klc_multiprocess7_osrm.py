# -*- coding: utf-8 -*-
"""
Created on Tue Mar 21 11:47:32 2023
...
def calculate_distance(row1, row2):
    try:
        response = client.route(
            coordinates=[[row1['longitude'], row1['latitude']], [row2['longitude'], row2['latitude']]],
            overview=osrm.overview.full)
        duration = response['routes'][0]['duration']
        route_dist = response['routes'][0]['distance']/1000
        jumpdist1 = response['waypoints'][0]['distance']/1000
        jumpdist2 = response['waypoints'][1]['distance']/1000
        total_route_dist = route_dist+jumpdist1+jumpdist2
    except:
        duration = -1
        route_dist = -1
        jumpdist1 = -1
        jumpdist2 = -1
        total_route_dist = -1
    return [duration, route_dist, jumpdist1, jumpdist2, total_route_dist]

def calculate_distances(row2, df1, df_weight):
    distances = []
    bank = []
    distances_raw = []
    bank_raw = []
    ...
    while len(df1_filtered) < 150 or len(caixa) == 15:
        geo_filter += 0.1
        df1_filtered = df1.loc[(abs(df1['latitude'] - row2['latitude']) <= geo_filter) & (abs(df1['longitude'] - row2['longitude']) <= geo_filter)]
        caixa = df1_filtered[df1_filtered['bank'] == 'Caixa']
                
    for index1, row1 in df1_filtered.iterrows():
        distances_raw.append(calculate_distance(row1, row2)[4])
        bank_raw.append(row1['bank'])

    # Drop distances and banks that are -1 total_route_dist
    distances = [distances_raw[i] for i in range(len(distances_raw)) if distances_raw[i] >= 0]
    bank = [bank_raw[i] for i in range(len(bank_raw)) if distances_raw[i] >= 0]
...
def calculate_all_distances(df1, df2, df_weight):
    ...
    results = pool.starmap(calculate_distances, [(row2, df1, df_weight) for _, row2 in df2.iterrows()])
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    df3_file = 'C:/Users/mathe/Dropbox/RESEARCH/pix/pix-event-study/Stata/dta/dist_caixa/dist_caixa_multiprocess7_osrm.dta'
    df3_file = "/home/mcs038/Documents/Pix_regressions/stata/dist_caixa_multiprocess7_osrm.dta"
    ...
    df3 = calculate_all_distances(df1, df2, df_weight)
