...
foreach var_treat of global list_of_treat{
	...
	foreach y of global list_of_y {
		...
		* Event study
		...
		coefplot SENT, title("Event Study for ExtraCad") subtitle("reghdfe `y' ib7.id_dist#i.grupo#1.treat , absorb(id time_id) vce(cluster id) base", size(vsmall)) vertical yline(0, lcolor(black) lwidth(thin)) xline(7, lcolor(black) lwidth(thin) lpattern(dash)) levels(90)  graphregion(color(white)) baselevels xtitle("Months") xlab(, labsize(vsmall))  ytitle(`l`y'') coeflabels(1.* = "Nov 20" 2.* = "Dec 20" 3.* = "Jan 21" 4.* = "Feb 21" 5.* = "Mar 21" 6.* = "Apr 21" 7.* = "May 21" 8.* = "Jun 21" 9.* = "Jul 21" 10.* = "Aug 21" 11.* = "Sep 21" 12.* = "Oct 21" 13.* = "Nov 21" 14.* = "Dec 21" 15.* = "Jan 22" 16.* = "Feb 22" 17.* = "Mar 22" 18.* = "Apr 22" 19.* = "May 22" 20.* = "Jun 22" 21.* = "Jul 22" 22.* = "Aug 22" 23.* = "Sep 22" 24.* = "Oct 22" 25.* = "Nov 22", wrap(4)) drop(_cons *.id_dist#1.grupo#1.treat *.id_dist#3.grupo#1.treat) note("Treatment defined as Distance to the Closest Caixa > `l`var_treat''") baselevels
...
use "C:\Users\mathe\Dropbox\RESEARCH\Natural Disasters\dta\danos_informados_monthly_filled_flood.dta", clear
...
gen treat = !missing(date_flood)

summ dist_flood
gen shifted_dist_flood = dist_flood - r(min)
summ shifted_dist_flood if dist_flood == -1
local true_neg1 = r(mean)
local true_neg11 = r(mean)+1
summ shifted_dist_flood if dist_flood == 0

/*
set seed 1234
gen rand = runiform()
global list_of_y rand
*/
merge 1:m mun_cd time_id using "$dta\Pix_PF_adoption.dta"

foreach y of global list_of_y {
	* Event study
	est clear 
	eststo SENT:reghdfe `y' ib`true_neg11'.id_dist_flood, absorb(mun_cd time_id) vce(cluster mun_cd) base
	
	* Limit the graph to include fewer months. 
	coefplot SENT, title("Event Study for Floods") subtitle("reghdfe `y' ib`true_neg11'.id_dist_flood, absorb(mun_cd time_id) vce(cluster mun_cd) base", size(vsmall)) vertical yline(0, lcolor(black) lwidth(thin)) levels(90) xline(`true_neg11', lcolor(black) lwidth(thin) lpattern(dash)) graphregion(color(white)) xtitle("Months") xlab(, labsize(vsmall))  ytitle(`l`y'') note("90% Confidence Intervals Shown") rename(*.shifted_dist_flood = "") drop(_cons) baselevels
	graph export "$output\flood_`y'.png", replace
}
