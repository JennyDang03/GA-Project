...
capture log close
log using "$log\Reg_Ini_version10.log", replace 

*-------------------------------------------------------------------------------
* PART 1: Differences in Differences - with road dist
*-------------------------------------------------------------------------------
...
*global list_of_treat expected_distance dist_bank // road_dist_bank road_expected_distance
global list_of_types bf non_bf // non_bf cad extracad all
...
local version "v10extracontrols_road"
	foreach restrict of global list_restriction {
		...
		*---------------------------------------------------------------------------
			foreach type of global list_of_types {
				...
				* Event Study
				est clear 
				eststo SENT:reghdfe `y' ib`base'.id_dist##c.road_dist_caixa ib`base'.id_dist##c.road_expected_distance if `type' == 1, absorb(id time_id muni_cd#time_id) vce(cluster id) allbaselevels
				local reg_equation "reghdfe `y' ib`base'.id_dist##c.road_dist_caixa ib`base'.id_dist##c.road_expected_distance if `type' == 1, absorb(id time_id muni_cd#time_id) vce(cluster id) allbaselevels"
				
				* Publish Graph
				coefplot SENT, title(`"Event Study for `l`type''"') vertical yline(0, lcolor(black) lwidth(thin))  levels(90) graphregion(color(white)) baselevels xtitle("Months") xlab(, labsize(vsmall)) ytitle(`l`y'')  drop(_cons *expected_distance)  coeflabels(1.* = "Nov 20" 2.* = "Dec 20" 3.* = "Jan 21" 4.* = "Feb 21" 5.* = "Mar 21" 6.* = "Apr 21" 7.* = "May 21" 8.* = "Jun 21" 9.* = "Jul 21" 10.* = "Aug 21" 11.* = "Sep 21" 12.* = "Oct 21" 13.* = "Nov 21" 14.* = "Dec 21" 15.* = "Jan 22" 16.* = "Feb 22" 17.* = "Mar 22" 18.* = "Apr 22" 19.* = "May 22" 20.* = "Jun 22" 21.* = "Jul 22" 22.* = "Aug 22" 23.* = "Sep 22" 24.* = "Oct 22" 25.* = "Nov 22" 26.* = "Dec 22", wrap(4)) order(1.* 2.* 3.* 4.* 5.* 6.*) note(`"Regression of `l`y'' on Distance to Caixa, controlled by Expected Distance. 90% Confidence Intervals shown."', size(vsmall))
				graph export "$output\did_distcaixa_`y'_`type'_`restrict'_`version'.png", replace	
				* subtitle(`"`reg_equation'"', size(vsmall)) xline(`april2021', lcolor(black) lwidth(thin) lpattern(dash)) xline(`october2021', lcolor(black) lwidth(thin) lpattern(dash))
...
local version "v10_road_limited"

***********************************************
* Adicionar limitacoes de tempo
keep if time_id <= ym(2021,12) 
***********************************************
