...
capture log close
log using "$log\Reg_Ini_version6.log", replace 
...
global list_of_types non_bf bf cad extracad
...
keep if time_id >= ym(2020,11) & time_id <= ym(2021,10) 
...
foreach type of global list_of_types {
	foreach y of global list_of_y {
		* Event study
		est clear 
		eststo SENT:reghdfe `y' ib`base'.id_dist_flood if `type' == 1, absorb(id time_id) vce(cluster muni_cd) allbaselevels // Cluster at muni_cd because the treatment was taken at muni_cd
		* I tried absorbing i.muni_cd#i.time_id but it didnt work
		local reg_equation "reghdfe `y' ib`base'.id_dist_flood if `type' == 1, absorb(id time_id) vce(cluster muni_cd) allbaselevels"
		*absorb(i.muni_cd##i.time_id) vce(cluster muni_cd)
