...
* Jose, tipo_pessoa == 1 PF, tipo_pessoa == 0 PJ???


	use "$dta\Pix_individuo.dta" if tipo_pessoa == 1,replace 
	...
	* Jose, voce pode checar se esse resultado da certo e elimina so 87?
	* Eu testei com fake data e deu certo
	...
	* log_ trans_rec trans_sent value_rec value_sent
	...
	drop tipo_pessoa 
	drop log_trans_rec log_trans_sent log_value_rec log_value_sent 
...
*	use "$dta\Pix_individuo_cleaned1.dta", clear 
	
	merge m:1 muni_cd time_id using "$dta\flood_monthly_2020_2022.dta"
	keep if _merge == 3
	drop _merge
	* Limit to after Pix
	keep if time_id >= ym(2020,11) & time_id <= ym(2022,12) 
...
foreach var of varlist valor_cartao_credito - qtd_boleto {
	cap drop log_`var'
	gen log_`var'=log(`var' + 1)
}
